...
> Note: If you see any messages of severity ERROR, the run hasn't necessarily failed, but the metrics it produces may not be usable. You should investigate the error and re-run. The `--debug` flag will result in the creation of a `debug.log` file in the output directory, which may be useful for debugging but can get really big.
